...
code128(data){
  ...
  //GS w n
  this.append(Buffer.from([0x1d, 0x77]));
  this.append(Buffer.from([0x02]));//somatoria dos bytes abaixo

  //GS h n
  this.append(Buffer.from([0x1d, 0x68]));
  this.append(Buffer.from([0x50]));//somatoria dos bytes abaixo

  //1d 6b m (GS k '73 - codigo 128')
  this.append(Buffer.from([0x1d, 0x6b, 0x49]));

  this.append(Buffer.from([24]));//somatoria dos bytes abaixo

  //n (tamanho caracteres)
  // this.append(Buffer.from([]));

  //d1 e d2 (Define o codigo como  128C)
  this.append(Buffer.from([0x7B, 0x43])); //somar dois blocos
  ...
  data.forEach(element => {
    ...
    if(cont == 2){
      cont = 0;
      arrayFormatado[self.cont2] = self.decimalToHexString(self.string);
      self.string = "";
      self.cont2++;
    }
  });
console.log("data",data);
console.log("data formatado",arrayFormatado);
  //o numero tem que ser de dois em dois
  this.append(Buffer.from(arrayFormatado));//somar dois bytes de cima, e mais um a cada virgula




  //dados para impressao
  // this.append(Buffer.from('3587987'));
